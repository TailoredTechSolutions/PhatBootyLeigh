  ...
  function Lightfall(props) {
    ...
    React.useEffect(() => {
      ...
      const init = () => {
        ...
        } catch (e) { console.warn("Lightfall: WebGL unavailable", e); return null; }
      ...
      const start = () => { if (!disposed && !teardown) teardown = init(); };
      if (window.ogl) start();
      else window.addEventListener("ogl-ready", start, { once: true });
    ...
    return (
      <div
        ...
        style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden", ...(mixBlendMode ? { mixBlendMode } : null) }}
      />
    );
  }
