...
(function () {
    ...
    for (let i = 1; i < usedImages.length; i++) {
      if (usedImages[i].src === usedImages[i - 1].src) {
        for (let j = i + 1; j < usedImages.length; j++) {
          if (usedImages[j].src !== usedImages[i].src) { const t = usedImages[i]; usedImages[i] = usedImages[j]; usedImages[j] = t; break; }
        }
      }
    }
  ...
  function DomeGallery(props) {
    const {
      ...
      padFactor = 0.25, overlayBlurColor = "#0a0a14",
    ...
    useEffect(() => {
      ...
      const onDown = (e) => {
        ...
        lastMoveRef.current = { t: performance.now(), x: e.clientX, y: e.clientY, vx: 0, vy: 0 };
    ...
    useEffect(() => {
      ...
      const close = () => {
        ...
        const originalPosRel = { left: originalPos.left - rootRect.left, top: originalPos.top - rootRect.top, width: originalPos.width, height: originalPos.height };
        ...
        if (originalImg) { const img = originalImg.cloneNode(); img.style.cssText = "width:100%;height:100%;object-fit:cover;"; anim.appendChild(img); }
        overlay.remove(); rootRef.current.appendChild(anim); void anim.getBoundingClientRect();
        requestAnimationFrame(() => {
          anim.style.left = originalPosRel.left + "px"; anim.style.top = originalPosRel.top + "px";
          anim.style.width = originalPosRel.width + "px"; anim.style.height = originalPosRel.height + "px"; anim.style.opacity = "0";
        });
        const cleanup = () => {
    ...
    const openItemFromElement = useCallback((el) => {
      ...
      const mainR = mainRef.current?.getBoundingClientRect(), frameR = frameRef.current?.getBoundingClientRect();
      ...
      const vsx = isFinite(sx0) && sx0 > 0 ? sx0 : 1, vsy = isFinite(sy0) && sy0 > 0 ? sy0 : 1;
      ...
      if (wantsResize) {
        const onFirstEnd = (ev) => {
          ...
          const tempWidth = openedImageWidth || `${frameR.width}px`, tempHeight = openedImageHeight || `${frameR.height}px`;
    ...
    return (
      <div ref={rootRef} className="sphere-root" style={{
        ...
        ["--tile-radius"]: imageBorderRadius, ["--enlarge-radius"]: openedImageBorderRadius, ["--image-filter"]: grayscale ? "grayscale(1)" : "none",
      }}>
